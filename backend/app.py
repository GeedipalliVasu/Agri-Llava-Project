...
#     "FRONTEND_ORIGINS",
#     "http://localhost:5173,http://localhost:5175,http://127.0.0.1:5175",
# ).split(",")
# CORS(
#     app,
...
#     heatmap = features[0].sum(dim=0).detach().cpu().numpy()
...
#     # Otsu threshold to get diseased mask
...
#             # Threshold the heatmap to obtain binary diseased regions (Otsu)
#             try:
#                 # Ensure heatmap_uint8 exists and create a binary mask using Otsu
#                 _, mask = cv2.threshold(
#                     heatmap_uint8, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
#                 )

#                 # Morphological opening to remove small noisy blobs
...
#                 min_area = max(100, int(0.002 * mask.shape[0] * mask.shape[1]))
#                 for cnt in contours:
#                     area = cv2.contourArea(cnt)
#                     if area >= min_area:
#                         cv2.drawContours(cleaned, [cnt], -1, 255, thickness=cv2.FILLED)

#                 # Create red overlay for cleaned mask
#                 red_overlay = np.zeros_like(img_np)
#                 red_overlay[:, :] = (0, 0, 255)

#                 seg_vis = img_np.copy()
#                 alpha_seg = 0.45
#                 seg_vis[cleaned > 0] = cv2.addWeighted(
#                     img_np[cleaned > 0],
#                     1 - alpha_seg,
#                     red_overlay[cleaned > 0],
#                     alpha_seg,
#                     0,
#                 )

#                 seg_filename = f"{uuid.uuid4()}_seg.jpg"
#                 seg_save_path = os.path.join(PREDICTION_FOLDER, seg_filename)
#                 cv2.imwrite(seg_save_path, seg_vis)

#                 segmentation_path = os.path.join(
#                     "static", "predictions", seg_filename
#                 ).replace("\", "/")
...
#             # Segmentation: Otsu + morphological cleaning + small component removal
#             try:
#                 _, mask = cv2.threshold(
#                     heatmap_uint8, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
#                 )
...
#     # Determine week index as sequential count per farmer
#     try:
#         existing_count = timelines_collection.count_documents({"farmer_id": farmer_id})
#     except Exception:
#         existing_count = 0
...
#     try:
#         timelines_collection.insert_one(entry)
#     except Exception as e:
#         print(f"Error saving timeline entry: {e}")

#     # Compute trend from last two severities for this farmer
#     try:
#         records = list(
...
#         if uid:
#             query["user_id"] = uid
#         records = list(timelines_collection.find(query, {"_id": 0}))
#         records.sort(key=lambda r: r.get("timestamp", ""))
#         severities = [float(r.get("severity_percent", 0.0)) for r in records]
#         trend = trend_from_series(severities)
#         return jsonify(
